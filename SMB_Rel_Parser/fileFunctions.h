...
namespace RELPatch {
	...
	inline uint32_t readBigInt(std::ifstream &fileStream) {
		char bytes[4];
		fileStream.read(bytes, 4);
		return (bytes[0] | bytes[1] | bytes[2] | bytes[3]);
	}

	inline uint16_t readBigShort(std::ifstream &fileStream) {
		char bytes[2];
		fileStream.read(bytes, 2);
		return (bytes[0] | bytes[1]);
	}

	inline uint8_t readBigByte(std::ifstream &fileStream) {
	...
	inline void writeBigShort(std::ofstream fileStream, uint16_t value) {
		char bytes[2];
		bytes[2] = (value >> 8);
		bytes[3] = (value);
